...
def _fn_params_to_ordered_dict(func, *args, **kwargs):
    ...
    pos_or_named_or_vargs_ordered_names = (
        pos_or_named_ordered_names + [spec.varargs]
        if varargs_dict
        else pos_or_named_ordered_names
    )

    pos_or_named_or_vargs_or_named_only_ordered_names = (
        pos_or_named_or_vargs_ordered_names + list(spec.kwonlyargs)
    )
    ...
    def __call__(self, func):
        ...
        def impl(*args, **kw):
            ...
            params_string = self.params_separator.join(
                list(map(derepresent, params)) if self.derepresent_params else params
            )
