...
class ProfilePage:
    ...
    def add_empty_category(self):
        browser.element("#category").press_enter()


    def edit_category(self, name, new_name):
        browser.all('[role="button"]').element_by(have.exact_text(name)).element(
            ".."
        ).element('[aria-label="Edit category"]').click()
    ...
    def archive_category(self, name):
        browser.all('[role="button"]').element_by(have.exact_text(name)).element(
            ".."
        ).element('[aria-label="Archive category"]').click()

        browser.element('[role="dialog"]').all('[type="button"]').element_by(
            have.exact_text("Archive")
        ).click()

    def unarchive_category(self, name):
        browser.all('span').element_by(have.text('Show archived')).click()
        browser.all('[role="button"]').element_by(have.exact_text(name)).element(
            ".."
        ).element('[aria-label="Unarchive category"]').click()

    def should_have_category(self, name):
        browser.all('[role="button"]').element_by(have.exact_text(name)).should(
            be.present
        )

    def category_should_be_absent(self, name):
        browser.all('[role="button"]').element_by(have.exact_text(name)).should(
            be.absent
        )
